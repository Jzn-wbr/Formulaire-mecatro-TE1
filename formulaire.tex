...
\newcommand{\metboxback}{white}
\newcommand{\metboxframe}{black}

\newtcolorbox{metbox}[1]{%
  colback=\metboxback,
  colframe=\metboxframe,
  boxrule=0.4pt,
...
\renewcommand{\metboxback}{blue!4}
\renewcommand{\metboxframe}{blue!65!black}
...
\medskip
\textbf{Conversion électromécanique (chap. 4)}

$W_{el}$ : énergie électrique [J] \\
$W_{mec}$ : énergie mécanique [J] \\
$W_{th}$ : énergie thermique (pertes) [J] \\
$W_{mag}$ : énergie magnétique [J] \\
$W'_{mag}$ : co-énergie magnétique [J] \\
$F_x$ : force électromagnétique selon $x$ [N] \\
$T$ : couple électromagnétique [N\,m] \\
$F_{res}$ : force résistante [N] \\
$T_{res}$ : couple résistant [N\,m] \\
$m$ : masse mobile [kg] \\
$J$ : moment d'inertie [kg\,m$^2$] \\
$C_v$ : coefficient de frottement visqueux [N\,s/m] \\
$G$ : conductance électrique [S] \\
$u_j$ : tension du circuit $j$ [V] \\
$i_j$ : courant du circuit $j$ [A] \\
$R_j$ : résistance du circuit $j$ [$\Omega$]

\end{metbox}

%====================================================
\begin{metbox}{Grandeurs (suite)}

$\Psi_j$ : flux totalisé du circuit $j$ [Wb] \\
$L_{jp}$ : inductance (propre/mutuelle) entre $j,p$ [H] \\
$\Lambda_{jp}$ : perméance (propre/mutuelle) entre $j,p$ [H] \\
$F_m$ : force généralisée selon $x_m$ [N] \\
$x_m$ : coordonnée généralisée [m] ou [rad] \\
$\theta_j=n_ji_j$ : potentiel magnétique du circuit $j$ [A] \\
$\theta_a=H_cl_a$ : potentiel magnétique d'un aimant [A] \\
$H_c$ : champ coercitif [A/m] \\
$B_r$ : induction rémanente [T] \\
$\mu_{ra}$ : perméabilité relative de l'aimant [-] \\
$K_F$ : constante de force [N/A] \\
$K_U$ : constante de tension induite [V\,s/m] \\
$K$ : constante de conversion électromécanique \\
$u_{im}$ : tension induite de mouvement [V]
...
\renewcommand{\metboxback}{green!4}
\renewcommand{\metboxframe}{green!45!black}
...
\renewcommand{\metboxback}{orange!6}
\renewcommand{\metboxframe}{orange!70!black}
...
%====================================================
\begin{metbox}{Équilibre stable / instable}
...
Critère local (linéaire) :
\[
\left.\frac{dF_{tot}}{dx}\right|_{x_{eq}}<0
\Rightarrow \text{stable},
\qquad
\left.\frac{dF_{tot}}{dx}\right|_{x_{eq}}>0
\Rightarrow \text{instable}
\]

Equivalent énergétique :
\[
\frac{dW}{dx}=0,\quad
\frac{d^2W}{dx^2}>0 \text{ stable},\;
\frac{d^2W}{dx^2}<0 \text{ instable}
\]

\begin{center}
\includegraphics[width=0.78\linewidth]{assets/figures/stable_instable.jpeg}
\end{center}
